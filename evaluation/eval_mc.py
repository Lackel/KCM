...
def read_pred_file(input_file):
    answers = [json.loads(q) for q in open(os.path.expanduser(input_file), "r")]
    preds = {}
    # with open(input_file, "r") as fin:
    for answer in answers:
        preds.update({answer['data_id']:answer['prediction']})
    return preds
...
def main():
    ...
    if args.input_file_2 is None:
        ...
        for data in dataset:
            ...
            temp.append(flag)
            cnt += 1
        # np.save('/workspace/mc_result.npy', temp)
        print(f"Accuracy: {cnt_correct / cnt}")
    else:
        eq = np.load('/workspace/mc_result.npy', allow_pickle=True)
        ...
        for data in dataset:
            ...
            if flag and not eq[cnt]:
                cnt_conflict += 1
            cnt += 1
        # print(sum(~eq))
        # print(f"Conflict Rate: {cnt_conflict / cnt}")
        print(f"Conflict Rate: {cnt_conflict / sum(~eq)}")
