...
def sample(
    ...
    **model_kwargs,
    ...
    if use_cd:
        model_kwargs_cd['attention_mask'] = torch.ones(model_kwargs.get("images_cd").shape[0], model_kwargs.get("images_cd").shape[1], device=model_kwargs.get("images_cd").device)
    ...
    while True:
        ...
        if use_cd:
            ...
            cd_beta = model_kwargs.get("cd_beta") if model_kwargs.get("cd_beta") is not None else 0.7
        
            entropy = get_entropy(next_token_logits)
            entropy_cd = get_entropy(next_token_logits_cd)
            diffs = (1 + entropy_cd) * next_token_logits + (1 + entropy) * next_token_logits_cd
        ...
        if return_dict_in_generate:
            ...
            if output_attentions:
                ...
                # import numpy as np
                # np.save("output_31_layers.npy", [decoder_attentions[-1][i].cpu().numpy() for i in range(32)])
                # print(decoder_attentions[-1][0][0][0][-1][35:611])
                # break
...
def exponential_weighted_sum_dim1(tensor, lambda_=1.0):
    H, W, D = tensor.shape
    weights = torch.exp(-lambda_ * (torch.arange(W, dtype=tensor.dtype, device=tensor.device)))
    weights = weights.flip(0)
    return torch.tensordot(weights, tensor, dims=([-1], [1]))