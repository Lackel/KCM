...
def eval_model(args):
    ...
    for line in tqdm(questions):
        ...
        prompt_len = len(tokenizer(' Context: ', return_tensors="pt", add_special_tokens=False).input_ids[0])
        context_len = len(tokenizer(context, return_tensors="pt", add_special_tokens=False).input_ids[0])


        prompt = 'You are an expert at question answering. Given the question, please output the answer. No explanation aor further questions. Question: ' + question +  ' Context: ' + context + ' Short answer: '

        prompt1 = 'You are an expert at question answering. Given the question, please output the answer. No explanation or further questions. Question: ' + question + ' Short answer: '
        ...
        with torch.inference_mode():
            outputs = model.generate({"image": image_tensor, "prompt": prompt},
                ...
                images_cd=prompt1, cd_beta = args.cd_beta, temperature=args.temperature, max_length=10, img_start_idx=0, img_end_idx=32, question_len = question_len, prompt_len = prompt_len, context_len = context_len, ret_sim = ret_sim, att_alpha = args.att_alpha)
