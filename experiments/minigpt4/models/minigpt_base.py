...
class MiniGPTBase(BaseModel):
    ...
    def generate(
        ...
        if images_cd is not None:
            # VCD & AGLA
            # img_embeds_cd, atts_img_cd = self.encode_img(images_cd.to(self.device))
            # image_lists_cd = [[image_emb[None]] for image_emb in img_embeds_cd]
            # batch_embs_cd = [self.get_context_emb(text, img_list) for text, img_list in zip(texts, image_lists_cd)]

            batch_embs_cd = [self.get_context_emb(text, img_list) for text, img_list in zip(images_cd, image_lists)]
            batch_size = len(batch_embs_cd)
            max_len = max([emb.shape[1] for emb in batch_embs_cd])
            emb_dim = batch_embs_cd[0].shape[2]
            dtype = batch_embs_cd[0].dtype
            device = batch_embs_cd[0].device

            embs_cd = torch.zeros([batch_size, max_len, emb_dim], dtype=dtype, device=device)
            attn_mask_cd = torch.zeros([batch_size, max_len], dtype=torch.int, device=device)
            
            for i, emb in enumerate(batch_embs_cd):
                emb_len = emb.shape[1]
                embs_cd[i, -emb_len:] = emb[0]
                attn_mask_cd[i, -emb_len:] = 1
        else:
            embs_cd = None
            attn_mask_cd = None
