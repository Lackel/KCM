    ...
    def generate(
        ...
        if images_cd is not None:
            ...
            if self.qformer_text_input:
                ...
                query_atts_cd = torch.ones(query_tokens.size()[:-1], dtype=torch.long).to(image.device)
                Qformer_atts_cd = torch.cat([query_atts_cd, text_Qformer_cd.attention_mask], dim=1)

            # For video data
            if image.dim() == 5:
                inputs_llm_cd, atts_llm_cd = [], []
                for j in range(image.size(2)):
                    this_frame = image[:,:,j,:,:]
                    with self.maybe_autocast():
                        frame_embeds = self.ln_vision(self.visual_encoder(this_frame))
                    ...
                    if self.qformer_text_input:
                        frame_query_output_cd = self.Qformer.bert(
                    ...
                    frame_inputs_llm_cd = self.llm_proj(frame_query_output_cd.last_hidden_state[:,:query_tokens.size(1),:])
                    frame_atts_llm_cd = torch.ones(frame_inputs_llm_cd.size()[:-1], dtype=torch.long).to(image.device)
                    inputs_llm_cd.append(frame_inputs_llm_cd)
                    atts_llm_cd.append(frame_atts_llm_cd)
                inputs_llm_cd = torch.cat(inputs_llm_cd, dim=1)
                atts_llm_cd = torch.cat(atts_llm_cd, dim=1)
            else:
                with self.maybe_autocast():
                    image_embeds = self.ln_vision(self.visual_encoder(image))
                image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
        ...
        with self.maybe_autocast():
            ...
            outputs = self.llm_model.generate(
                ...
                attention_mask_cd = attention_mask_cd if images_cd is not None else None,
                cd_beta=cd_beta if cd_beta is not None else None,
                img_start_idx = img_start_idx if img_start_idx is not None else None,
                img_end_idx = img_end_idx if img_end_idx is not None else None,
